...
--- @class HUD
--- @field RenderImages Render2dInstance[]
local STATIC = CNC_RENEGADE.Hud
STATIC.RenderImages = {}
...
concommand.Add( "AddPowerup", function()
    STATIC.AddDataLink()
    STATIC.AddShieldGrant( 100 )
    STATIC.AddHealthGrant( 40 )
    STATIC.AddObjective( objectiveManager.ObjectiveType.TYPE_PRIMARY )
    STATIC.AddObjective( objectiveManager.ObjectiveType.TYPE_SECONDARY )
    STATIC.AddShieldUpgradeGrant( 10 )
    STATIC.AddHealthUpgradeGrant( 10 )
    STATIC.AddMapReveal()
end )
...
local function RenderPowerupList( powerupList, frameTime, animationTimer, isRightList )
    ...
    for index, powerup in ipairs( powerupList ) do
        if index > STATIC.MaxIcons then break end
...
--- Emulates the behavior of WeaponBoxRenderer in Weapon_Init() within Code/Combat/Hud.cpp
function STATIC.WeaponBoxRender()
    local boxUv = robustclass.New( "Renegade_Rect", STATIC.WeaponBoxUvUpperLeft, STATIC.WeaponBoxUvLowerRight )
    local drawBox = robustclass.New( "Renegade_Rect", boxUv )
    ...
    surface.SetMaterial( STATIC.Materials.Hud.Main )
...
-- Reticle
STATIC.ReticleWidth = ( 64 / 640 )
STATIC.ReticleWidth = ( 64 / 480 )
